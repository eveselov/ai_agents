...
Add a `search_web` tool via the Tavily API using the same `@tool` decorator. The agent can now answer questions that require current knowledge. The mechanics are unchanged — one more Python function, one more string returned. Point out: database query → string, API call → string, file read → string. The model's world is entirely made of strings.

**Part 8 — Provider-Side Web Search**
OpenAI (and Anthropic, Google) offer built-in search as a declared tool type: `{"type": "web_search_preview"}`. No schema to write, no dispatch to implement — the provider runs the search on their infrastructure. Uses OpenAI's Responses API. Contrast with Part 7: the string interface is identical, only the dispatch location changed. The provider is running your Part 5 loop on their servers and handing back only the final answer.
...
- External APIs as tools: any capability reduces to string in, string out
- Provider-side tool dispatch: convenience vs. visibility trade-off; the hidden loop
...
4. With provider-side search, you get one HTTP call back. What did you give up compared to running the loop yourself?
5. The model receives a tool result as plain text. What happens if that text is adversarial — e.g., a web page that says "Ignore all previous instructions"?
...
- Part 8 is best framed as a question: *"How many autoregression passes happened inside that one API call?"* The answer (two: one to emit the tool call, one to synthesize the result) surprises most people. The convenience is real; so is the opacity.
